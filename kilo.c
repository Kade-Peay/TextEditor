...
// Declare this here so that it compiles, but write the actual function later
...
void editorOpen(char *filename) {
    ...
    E.dirty = 0; // set the changes tracker to 0 on open
} 
    ...
    if (fd != -1) {
        if (ftruncate(fd, len) != -1) {
            if (write(fd, buf, len) == len) {
                close(fd);
                free(buf);
                E.dirty = 0; // set changes tracker back to 0 when the file has been saved
...
// empty buffer to act as a constructor for abuf
